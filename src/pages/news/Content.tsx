...
const LatestBigCard = ({ item, type, onClick }: { item: ContentItem; type: 'news' | 'article'; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-bold text-lg text-gray-800 line-clamp-3 mb-4">{title}</h3>
...
const LatestSmallCard = ({ item, type, onClick }: { item: ContentItem; type: 'news' | 'article'; onClick?: () => void }) => {
  const imageUrl = item.file || '/assets/img/placeholder.svg';
  const title = type === 'news' ? item.cbtitl : item.cetitl;
  const author = type === 'news' ? item.cbchid : item.cechid;
  const dateStr = type === 'news' ? item.cbpsdt : item.cepsdt;
...
const OtherCard = ({ item, type, onClick }: { item: ContentItem; type: 'news' | 'article'; onClick?: () => void }) => {
  const imageUrl = item.file || '/assets/img/placeholder.svg';
  const title = type === 'news' ? item.cbtitl : item.cetitl;
  const author = type === 'news' ? item.cbchid : item.cechid;
  const dateStr = type === 'news' ? item.cbpsdt : item.cepsdt;
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        if (selectedCategory) {
          filteredData = data.filter((item: ContentItem) => item.category === selectedCategory);
        }
